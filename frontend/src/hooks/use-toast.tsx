...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const remove = (id: string) => setToasts((current) => current.filter((item) => item.id !== id));
  ...
  return (
    <ToastContext.Provider value={value}>
      ...
      <div className="fixed right-4 top-4 z-50 space-y-3">
        {toasts.map((toast) => (
          <div key={toast.id} className="w-80 rounded-2xl border border-sky-400/20 bg-slate-950/90 p-4 shadow-glow backdrop-blur">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="font-semibold text-white">{toast.title}</p>
                {toast.description ? <p className="mt-1 text-sm text-slate-300">{toast.description}</p> : null}
              </div>
              <button className="text-slate-400" onClick={() => remove(toast.id)}>
                ×
              </button>
            </div>
          </div>
